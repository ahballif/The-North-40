                ...
                ToolbarItemGroup(placement: .bottomBar) {
                    HStack {
                        ...
                        .sheet(isPresented: $showingEditEventSheet) {
                            EditEventViewWatch(isScheduled: false, eventType: ["To-Do", "checklist"])
...
//                }
    ...
    private func toDoCell(_ event: N40Event) -> some View {
        ...
        if (UserDefaults.standard.bool(forKey: "colorToDoList") && UserDefaults.standard.bool(forKey: "showEventsInGoalColor") && event.getAttachedGoals.count > 0) {
            //draw with the goal color
            cellColor = Color(hex: event.getAttachedGoals.first!.color)?.opacity(0.5) ?? .clear
            
        }
        if (UserDefaults.standard.bool(forKey: "colorToDoList") && !(UserDefaults.standard.bool(forKey: "showEventsInGoalColor") && event.getAttachedGoals.count > 0) && (UserDefaults.standard.bool(forKey: "showEventsWithPersonColor") && event.getFirstFavoriteColor() != nil)) {
            //draw with the person color
            cellColor = Color(hex: event.getFirstFavoriteColor()!)?.opacity(0.5) ?? .clear
        }
        if (UserDefaults.standard.bool(forKey: "colorToDoList") && !(UserDefaults.standard.bool(forKey: "showEventsInGoalColor") && event.getAttachedGoals.count > 0) && !(UserDefaults.standard.bool(forKey: "showEventsWithPersonColor") && event.getFirstFavoriteColor() != nil) && ((UserDefaults.standard.bool(forKey: "showEventsInGoalColor") || UserDefaults.standard.bool(forKey: "showEventsWithPersonColor")) && UserDefaults.standard.bool(forKey: "showNoGoalEventsGray"))) {
            //draw with the gray color
            cellColor = Color(hex: "#b9baa2")?.opacity(0.5) ?? .clear
        }
        if (UserDefaults.standard.bool(forKey: "colorToDoList") && !(UserDefaults.standard.bool(forKey: "showEventsInGoalColor") && event.getAttachedGoals.count > 0) && !(UserDefaults.standard.bool(forKey: "showEventsWithPersonColor") && event.getFirstFavoriteColor() != nil) && !((UserDefaults.standard.bool(forKey: "showEventsInGoalColor") || UserDefaults.standard.bool(forKey: "showEventsWithPersonColor")) && UserDefaults.standard.bool(forKey: "showNoGoalEventsGray"))) {
            //draw with the original color
            cellColor = Color(hex: event.color)?.opacity(0.5) ?? .clear
        }
        ...
        return ZStack {
            ...
            HStack {
                ...
                NavigationLink(destination: EditEventViewWatch(editEvent: event), label: {
                    VStack {
                        HStack{
                            Text(event.name)
                            Spacer()
                    ...
                    if (toDo.status == 2) {
                        withAnimation {
                            ...
                            if toDo.repeatOnCompleteInDays > 0 && toDo.status != N40Event.UNREPORTED && (toDo.eventType == N40Event.TODO_TYPE || toDo.eventType == N40Event.REPORTABLE_TYPE) {
                                ...
                                // watch doesn't do syncing with calendar
                            }
